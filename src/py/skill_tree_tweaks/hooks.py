...
def apply_skill_tree_changes(movie: UObject):
    ...
    def apply_skill_flash(
        ...
        if is_unlocked:
            cur_points = cur_skill_val + skill_augment
            max_points = max_skill_val
            from . import OPT_LIMIT_MAX_POINTS

            if not OPT_LIMIT_MAX_POINTS.value:
                max_points += skill_augment

            movie.SetVariableString(
                f"{p}.points.htmlText",
                f"{cur_points}/{max_points}",
            )
        else:
...
@hook(
    hook_func="WillowGame.SkillTreeGFxHelper:Flash_SetCurrentSelectionDescription",
    hook_type=Type.POST,
)
def hook_skill_tree_selection_after(
    ...
    skill_augment = (
        0 if skill_def not in augmented_skills else augmented_skills[skill_def]
    )
    ...
    def get_description(grade: int) -> str:
        nonlocal obj
        nonlocal skill_def
        return obj.GetSkillDescriptionForGrade(
            skill_def, grade, obj.WPCOwner, False
        ).strip()
    ...
    movie.SetVariableString(
        "skills.description.htmlText", f"{desc}\n{current_grade_desc}{next_grade_desc}"
    )
